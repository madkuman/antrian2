...
class BaseController extends Controller
{
	...
	/* protected function hasPermissionPrefix($action, $table_column = null, $column_check = null) {
		...
		foreach ($this->userPermission as $val) 
		{
			...
			if ($user_action == $action || $user_action == $action . '_all') {
				$allowed = 'all';
				break;
			} else if ($user_action == $action . '_own') {
				$allowed = 'own';
			}
		}
	...
	protected function permissionErrorMessage($type) {
		$permission_title = ['read' => 'melihat data', 'create' => 'menambah data', 'update' => 'mengubah data', 'delete' => 'menghapus data'];
		return 'Role Anda tidak memiliki hak untuk ' . $permission_title[$type];
	}
}